...
class YOLO_Loss():
    ...
    def loss(self):
        ...
        for sample in range(self.predictions.shape[0]):
            ...
            self.final_loss += self.lambda_coord*mid_loss + self.lambda_coord*dim_loss + self.lambda_noobj*conf_loss_noobj + conf_loss + class_loss
    ...
    def find_best_box(self, sample, cell_h, cell_w):
        ...
        t_box_coords = MidtoCorner(self.targets[sample, cell_h, cell_w, 1:5], cell_h, cell_w, self.cell_dim)
        
        best_box = 0
        max_iou = 0.
        for box in range(self.num_boxes):
            # Transform the box coordinates into the corner format
            p_box_coords = MidtoCorner(self.predictions[sample, cell_h, cell_w, 1+box*5:5+box*5], cell_h, cell_w, self.cell_dim)
